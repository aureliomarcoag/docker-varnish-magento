...
#DOMAIN_MAPPING="domain.com:MAGE_TYPE:MAGE_CODE"
MAGE_CODE_TYPE_MAPPING=

IFS='|' ; for store in ${DOMAIN_MAPPING} ; do
    IFS=':'
    values=(${store})
    MAGE_CODE_TYPE_MAPPING+="    if (req.http.host == \"${values[0]}\") {\n"
    MAGE_CODE_TYPE_MAPPING+="        set req.http.x-mage-type = \"${values[1]}\";\n"
    MAGE_CODE_TYPE_MAPPING+="        set req.http.x-mage-code = \"${values[2]}\";\n"
    MAGE_CODE_TYPE_MAPPING+="    }\n"
done
...
acl purge {
    # CMAGE: Allow private subnets to purge
